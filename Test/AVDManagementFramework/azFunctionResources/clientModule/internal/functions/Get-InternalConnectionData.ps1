...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'AVDManagementFramework.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'AVDManagementFramework.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'AVDManagementFramework.Client.ProtectedToken'
