...
$parentModule = 'AVDManagementFramework'
if (-not $ModuleName) { $ModuleName = 'AVDManagementFramework.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\AVDManagementFramework'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\AVDManagementFramework.psd1"
$prereqHash = @{
