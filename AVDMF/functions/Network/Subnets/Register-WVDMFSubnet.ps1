function Register-WVDMFSubnet {
    ...
    process {
        ...
        $addressPrefix = (New-WVDMFSubnetRange -AddressSpace $addressSpace -NewSubnetMaskBits $subnetMask -ErrorAction 'Stop').AddressPrefix
        #endregion: Calculate subnet range and prefix

        $resourceName = New-WVDMFResourceName -ResourceType 'Subnet' -ParentName $NamePrefix -AddressPrefix $addressPrefix
        $resourceID = "$VirtualNetworkID/subnets/$resourceName"

        $script:Subnets[$resourceName] = [PSCustomObject]@{
            PSTypeName         = 'WVDMF.Network.Subnet'
