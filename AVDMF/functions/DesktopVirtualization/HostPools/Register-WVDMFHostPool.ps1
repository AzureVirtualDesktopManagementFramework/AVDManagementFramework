function Register-WVDMFHostPool {
    [CmdletBinding()]
    param (
    ...
    process {
        $ResourceName = New-WVDMFResourceName -ResourceType 'HostPool' -AccessLevel $AccessLevel -HostPoolType $PoolType
        $resourceGroupName = New-WVDMFResourceName -ResourceType "ResourceGroup" -ResourceCategory 'HostPool' -AccessLevel $AccessLevel -HostPoolType $PoolType
        Register-WVDMFResourceGroup -Name $resourceGroupName -ResourceCategory 'HostPool'
        ...
        $subnetID = Register-WVDMFSubnet @subnetParams -PassThru
        ...
        Register-WVDMFFileShare -Name $resourceName.ToLower() -StorageAccountName $storageAccount.Name -ResourceGroupName $storageAccount.ResourceGroupName
        ...
        $script:HostPools[$ResourceName] = [PSCustomObject]@{
            PSTypeName           = 'WVDMF.DesktopVirtualization.HostPool'
        ...
        Register-WVDMFApplicationGroup -HostPoolName $resourceName -ResourceGroupName $resourceGroupName -HostPoolResourceId $resourceID
        ...
        for ($i = 1; $i -le $sessionHostsCount; $i++) {
            ...
            Register-WVDMFSessionHost -ResourceGroupName $resourceGroupName -AccessLevel $AccessLevel -HostPoolType $PoolType -HostPoolInstance $hostPoolInstance -InstanceNumber $i -VMTemplate $script:VMTemplates[$VMTemplate] @SessionHostParams
        }
