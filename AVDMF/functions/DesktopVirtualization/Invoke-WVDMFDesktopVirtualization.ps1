function Invoke-WVDMFDesktopVirtualization {
    [CmdletBinding()]
    param (
    ...
    foreach ($rg in $script:ResourceGroups.Keys) {
        if ($script:ResourceGroups[$rg].ResourceCategory -eq 'HostPool') {
            $templateParams = Initialize-WVDMFDesktopVirtualization -ResourceGroupName $rg -ResourceCategory 'HostPool'
    ...
    foreach ($rg in $script:ResourceGroups.Keys) {
        if ($script:ResourceGroups[$rg].ResourceCategory -eq 'Workspace') {
            $templateParams = Initialize-WVDMFDesktopVirtualization -ResourceGroupName $rg -ResourceCategory 'Workspace'
